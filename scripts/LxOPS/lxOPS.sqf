...
Lx_fnc_newObject = {
    ...
    // For some reason, with an alphabetically sorted list, the above command changes the selected item.  This resets it
    lbSetCurSel [24357, _InitialSelect];    
};
...
<t color='#ffffff' align = 'center' shadow = '1' shadowColor='#312f0d' size = '1.8' font='PuristaSemibold'>No lock</t>";
    ...
    (_dlg displayCtrl 24358) ctrlSetStructuredText parseText format [RedColor_Tag + "Selection set to: %1</t>", lbCurSel 24357];     
};
...
lxFilterObject = {
            ...
            for "_i" from 0 to (count (configFile >> "CfgVehicles") - 1) do {
                ...
                if (isClass(_objCfg)) then {
                    if (getNumber (_objCfg >> "scope") > 0) then {
                        _objects set [_idx, format["%1", configName (_objCfg)]];                                                
                    };
                    _idx = _idx + 1;                    
                };
            };
	...
	lbClear 24357;
	{
        ...
        _copiedTxt = _copiedTxt + str(_x) + " | " + str(_counter) + ", ";
        _counter = _counter + 1;
	} forEach _objects;
	...
	(_dlg displayCtrl 24358) ctrlSetStructuredText parseText "<t size='1.2' align='center' color='#ffffff'>Object list copied to clipboard</t>";
};
...
Lx_fnc_GotoObject = {
    _CurIndex = lbCurSel 24357;
    _JumpTo = parseNumber ctrlText 1402;
    
    if (_JumpTo < 0 || _JumpTo > lxNumObjs) then {
        _JumpTo = 1;
    };
    lbSetCurSel [24357, _JumpTo];
};
...
{
	_index = _ctrl lbAdd _x;
} forEach ["Alpha", "Building","Land vehicle","Air vehicle","Ship","All"];
lbSetCurSel [24354, 0];
